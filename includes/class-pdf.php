...
abstract class EU_Withdrawal_Button_PDF extends EU_Withdrawal_Button_Order_Status {
    protected function create_pdf_receipt(int $post_id,array $m): string {
        ...
        $lines=[ $this->t('email_subject'), $this->t('request_id').': #'.$post_id, $this->t('reference_code').': '.($m['reference_code'] ?? $post_id), $this->t('name').': '.$m['customer_name'], $this->t('email').': '.$m['customer_email'], $this->t('order').': '.$m['order_number'], $this->t('submitted_at').': '.$m['submitted_at'], $this->t('products').': '.implode('; ',(array)$m['products']), $this->t('declaration_label').': '.$this->t('declaration') ];
        $raw_text=implode("\n", $lines);
        if($this->pdf_requires_unicode($raw_text)){
            update_post_meta($post_id, '_ewb_pdf_receipt_skipped', 'unicode');
            return '';
        }
    ...
    protected function pdf_requires_unicode(string $text): bool { return preg_match('/[^\x09\x0A\x0D\x20-\xFF]/u', $text) === 1; }
